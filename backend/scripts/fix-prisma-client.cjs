...
// Script to remove problematic globalThis assignment from
...
if (fs.existsSync(clientPath)) {
  ...
  content = content.replace(
    ...
    /.*globalThis\[.*\].*path\.dirname\(fileURLToPath\(import\.meta\.url\)\);?.*\n?/g,
    '// Manual patch: removed problematic globalThis assignment\n',
  )
